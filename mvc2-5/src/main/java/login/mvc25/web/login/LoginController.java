...
import login.mvc25.domain.login.LoginService;
...
//    @PostMapping("/login")
    public String loginV2(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletResponse response) {
        ...
        sessionManager.createSessionMap(loginMember, response);
        return "redirect:/";
    }
...
//    @PostMapping("/login")
    ...
    /**
     * 서블릿이 제공하는 HttpSession 사용 로그 아웃
     */
    @PostMapping("/logout")
    ...
    /**
     * 로그인 이후 redirect처리, 로그인 HttpSession + 필터/인터셉터
     */
    @PostMapping("/login")
